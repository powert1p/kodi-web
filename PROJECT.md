...
Стартап: оффлайн матем-центр + AI-практика. Тему объясняет живой учитель; индивидуальную практику ведёт AI —
тренажёр ошибок (React PWA `/app/`): фото решения → поэтапная проверка → чат-тьютор; в перспективе live-планшет
и голос (**видение и роадмап: [docs/VISION.md](docs/VISION.md)**). Legacy learning-платформа (Flutter Web на `/`):
диагностика, адаптивная практика, экзамены, граф знаний. Монорепо + **FastAPI** backend, один Docker-образ.

- **Стек:** Python 3.11 + FastAPI + SQLAlchemy 2.0 async + asyncpg + PostgreSQL; React 19 PWA (`webapp/`); legacy Flutter Web (`frontend/`)
...
| Дизайн-токены React PWA (читать перед UI-работой) | [webapp/DESIGN_SYSTEM.md](webapp/DESIGN_SYSTEM.md) |
