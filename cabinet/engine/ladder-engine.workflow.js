...
const AUTHOR = [
  ...
  '',
  '# Три провала, которые тут реально допускали — проверь их в первую очередь',
  '- Дробление (нарушает п.1): «найди 20% от 25000» + «прибавь» = ОДНА ступень «цена выросла на 20%». «Раздели 3 на 5» + «переведи в %» = одна ступень «сколько процентов 3 от 5».',
  '- Двойной вопрос (нарушает п.3): «назови наименьшее И наибольшее» = два значения. Либо две ступени, либо одно дай готовым в условии и спрашивай второе.',
  '- Ответ/формула в варианте (нарушает п.4): варианты choose — это голые значения или короткие фразы. НЕ пиши в опции формулу решения («(24−16)÷2», «1/6−1/15», «=4»). Дай голые значения, один из которых = реальная ошибка ребёнка: например [8 / 4 / 20], где 8 — его ошибка.',
  '',
  '# Выход',
...
const CRITIC = [
  ...
  '4. Вопрос не содержит свой ответ. Нет искомого числа, готового выражения, подсказки верной стороны в формулировке И В ВАРИАНТАХ. Нарушение: «(½ от ¾)» в вопросе; вариант choose с формулой решения «(24−16)÷2» или «=4» (варианты должны быть голыми значениями [8 / 4 / 20]). Тест: понявший и непонявший ответили бы по-разному?',
...
const MODEL = _a.model || 'sonnet' // движок по умолчанию на Sonnet (плотнее автор + строже критик); переопредели args.model
log(`engine: получено задач = ${tasks.length}, модель агентов = ${MODEL || 'inherit (opus)'}`)
...
const results = await pipeline(
  ...
  (t) =>
    agent(AUTHOR + '\n\n' + taskBlock(t), {
      label: `author:${t.node_id}`, phase: 'Author', schema: LADDER_SCHEMA, effort: 'medium', model: MODEL,
    }),
  (ladder, t) =>
    ...
    agent(CRITIC + '\n\n' + taskBlock(t) + '\n\n=== ЛЕСЕНКА ОТ АВТОРА ===\n' + JSON.stringify(ladder, null, 1), {
      label: `critic:${t.node_id}`, phase: 'Critic', schema: CRITIC_SCHEMA, model: MODEL,
  ...
  (cr, t) => {
    ...
    return agent(
      ...
      { label: `revise:${t.node_id}`, phase: 'Revise', schema: LADDER_SCHEMA, effort: 'medium', model: MODEL },
