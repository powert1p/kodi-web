...
export const easierPool: Record<string, EasierRung> = {
  ...
  // Скорость течения (WP03): течение = половина разности скоростей
  stream_speed: {
    instruction:
      "Тренировка скилла. Два одинаковых течения вместе дают $10$ км/ч. Сколько км/ч одно течение? ($10\\div2$)",
    microSkill: "stream_speed",
    expected: "5",
  },
  // Возраст (EQ04): «в 3 раза меньше» — это деление, а не вычитание
  times_vs_subtract_younger: {
    instruction:
      "Зайдём проще. «В $3$ раза меньше, чем $12$» — это $12-3$ или $12\\div3$? Посчитай правильный вариант.",
    microSkill: "times_vs_subtract_younger",
    expected: "4",
  },
  // Возраст (EQ04): приведение подобных слагаемых
  combine_like_terms: {
    instruction: "Зайдём проще. Сложи и приведи подобные: $x+x+x$. Сколько $x$ получится?",
    microSkill: "combine_like_terms",
    expected: "3",
  },
    ...
    steps: [
      {
        ...
        instruction:
          "Теплоход прошёл 80 км по течению за 3 ч 20 мин. Это время в часах — сколько (дробью)?",
        expected: "10/3",
        reveal: "20 мин = 20/60 = 1/3 ч, значит 3 ч 20 мин = 10/3 ч.",
      },
      {
        n: 2,
        kind: "compute",
        microSkill: "speed_from_distance_time",
        instruction: "Скорость по течению: 80 км за 10/3 ч. Сколько км/ч?",
        expected: "24",
        reveal: "80 : (10/3) = 80 × 3/10 = 24 км/ч.",
      },
      {
        n: 3,
        kind: "choose",
        microSkill: "stream_speed_structure",
        instruction:
          "По течению теплоход разгоняется до 24 км/ч, а против — еле плывёт 16 км/ч. Эту разницу в скорости даёт течение. Оно тут спрятано один раз или два?",
        options: [
        ...
        expected: "o1",
        reveal:
          "По течению = собственная + течение, против = собственная − течение. Разность 24 − 16 = 8 — это (+течение) − (−течение) = два течения.",
      },
      {
        n: 4,
        kind: "compute",
        microSkill: "stream_speed",
        instruction:
          "Мы выяснили, что разность 8 км/ч — это два течения сразу. Сколько тогда одно течение?",
        expected: "4",
        reveal: "8 : 2 = 4 км/ч — это и есть скорость течения.",
      },
    ],
  },
  {
    ...
    steps: [
      {
        ...
        instruction:
          "В сумму возрастов входят: старший брат, младший брат, сестрёнка, отец. А самого Адилета мы в эту сумму кладём?",
        options: [
          { id: "o0", label: "да, кладём" },
          { id: "o1", label: "нет, только родню" },
        ],
        expected: "o0",
        reveal:
          "Адилет — тоже один из тех, чьи возрасты складывают: его x обязательно в сумме.",
      },
      {
        n: 2,
        kind: "choose",
        microSkill: "add_vs_multiply_older",
        instruction:
          "«Брат на 3 года старше Адилета» — это к возрасту Адилета прибавить или умножить на что-то?",
        options: [
          { id: "o0", label: "прибавить 3" },
          { id: "o1", label: "умножить на 3" },
        ],
        expected: "o0",
        reveal: "«На 3 старше» — сдвиг на 3 года: x+3. Умножение тут ни при чём.",
      },
      {
        n: 3,
        kind: "choose",
        microSkill: "times_vs_add_older",
        instruction:
          "«Отец в 3 раза старше Адилета» — это снова прибавить 3 или взять втрое больше?",
        options: [
          { id: "o0", label: "прибавить 3" },
          { id: "o1", label: "умножить на 3" },
        ],
        expected: "o1",
        reveal:
          "«В 3 раза старше» — это втрое больше: 3·x. Не путать с «на 3 старше» (x+3).",
      },
      {
        n: 4,
        kind: "choose",
        microSkill: "times_vs_subtract_younger",
        instruction:
          "«Сестрёнка в 3 раза младше Адилета» — её возраст это x минус 3 или x делить на 3?",
        options: [
          { id: "o0", label: "x − 3" },
          { id: "o1", label: "x : 3" },
        ],
        expected: "o1",
        reveal: "«В 3 раза младше» — втрое меньше: x/3. «На 3 младше» было бы x−3.",
      },
      {
        n: 5,
        kind: "compute",
        microSkill: "combine_like_terms",
        instruction:
          "Сложи все пять возрастов: x + (x+3) + (x−3) + x/3 + 3x. Сколько получится целых x?",
        expected: "6",
        reveal:
          "+3 и −3 взаимно уничтожаются, остаётся x + x + 3x = 6x (и ещё дробь x/3).",
      },
    ],
  },
  {
