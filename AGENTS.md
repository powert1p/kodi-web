...
Стартап: оффлайн матем-центр — тему объясняет живой учитель, индивидуальную практику ведёт AI (фото решения → поэтапная проверка → в перспективе live-планшет и голос). **Flutter Web + React PWA (тренажёр) + FastAPI**, монорепо, один Docker-образ, свой VPS.

## Где что (живые доки — читай вместо раскопок)
- **docs/VISION.md** — видение стартапа, роадмап этапов, несущие техпринципы. ЧИТАТЬ при любых продуктовых решениях.
...
- **docs/plan/current.md** — текущий план. **docs/decisions/** — ADR. **.Codex/rules/** — правила по слоям.
