#!/usr/bin/env node
// Гейт дизайн-системы v6 «Тетрадь чемпиона» (DESIGN-V6.md): ловит сырые значения
// прямо в исходниках, без внешних зависимостей. Все v6-токены (ink-пары, слои бумаги,
// клетка, тени) живут в tokens.css/fonts.css — гейт их НЕ трогает, но ловит любой сырой
// hex/px, просочившийся в компоненты (там — только var(--token) и Tailwind-утилиты).
//
// Проверки:
...
//
// Исключения (единственный источник сырых значений): theme/tokens.css и theme/fonts.css.
