...
class _DiagnosticPageState extends State<DiagnosticPage> {
  ...
  bool _isActiveSession(DiagnosticState state) =>
      state is DiagnosticQuestionReady || state is DiagnosticAnswerShown;
  ...
  PreferredSizeWidget _buildAppBar(
      BuildContext context, DiagnosticState state) {
    final l = AppLocalizations.of(context)!;
    return AppBar(
            ...
            Padding(
                ...
                child: Text(_fmtTime(state.elapsedSeconds),
                    style: TextStyle(
                        ...
                        color: state.elapsedSeconds > 120
                            ? AppColors.error
                            : Colors.grey[400],
  ...
  Widget _buildContent(BuildContext context, DiagnosticState state) {
    return switch (state) {
      DiagnosticError(:final message) =>
        _buildError(context, localizeError(context, message)),
  ...
  Widget _buildResumePrompt(
      BuildContext context, DiagnosticActiveSessionFound state) {
    final l = AppLocalizations.of(context)!;
    final modeName =
        state.activeMode == 'gaps' ? l.gapsCheck : l.examPreparation;
    ...
    return Column(children: [
      ...
      Container(
          ...
          child: Icon(Icons.pause_circle_filled_rounded,
              color: Colors.white, size: rs(context, 44))),
      ...
      Text(
          l.unfinishedDiagnosticInfo(modeName, state.activeTopicsTested,
              state.activeMaxTopics, state.activeQuestionsAsked),
      ...
      ClipRRect(
          ...
          child: LinearProgressIndicator(
              value: state.activeMaxTopics > 0
                  ? state.activeTopicsTested / state.activeMaxTopics
                  : 0,
              minHeight: 8,
  ...
  Widget _buildStart(BuildContext context) {
    ...
    return Column(children: [
      ...
      const SizedBox(height: 32),
      _ModeCard(
          title: l.diagnosticExamMode,
          subtitle: l.diagnosticExamSubtitle,
      ...
      const SizedBox(height: 12),
      _ModeCard(
          title: l.diagnosticGapsMode,
          subtitle: l.diagnosticGapsSubtitle,
  ...
  Widget _buildQuestion(BuildContext context, DiagnosticQuestionReady state) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ClipRRect(
          borderRadius: BorderRadius.circular(4),
          child: LinearProgressIndicator(
              value: state.maxTopics > 0
                  ? state.topicsTested / state.maxTopics
                  : 0,
              minHeight: 6,
              backgroundColor: AppColors.border,
      ...
      AnswerInput(
        ...
        onSubmit: () =>
            bloc.add(DiagnosticAnswerSubmitted(_controller.text.trim())),
  ...
  Widget _buildAnswerResult(BuildContext context, DiagnosticAnswerShown state) {
    ...
    return Column(children: [
      ...
      ResultCard(
        ...
        onReport: !ok
            ? () => showReportSheet(context, api, state.question['problem_id'],
                studentAnswer: _controller.text)
            : null,
      ),
      const SizedBox(height: 16),
      SizedBox(
          ...
          child: FilledButton.icon(
              ...
              icon: Icon(
                  hasNext ? Icons.arrow_forward_rounded : Icons.flag_rounded),
  ...
  Widget _buildResults(BuildContext context, DiagnosticFinished state) {
    ...
    return Column(children: [
      ...
      const SizedBox(height: 8),
      Text(summary,
          style: TextStyle(fontSize: rs(context, 14), color: Colors.grey[600]),
          textAlign: TextAlign.center),
      const SizedBox(height: 24),
      Row(children: [
        Expanded(
        ...
        Expanded(
            child: _ResultStat(
                label: l.gaps, value: '$failed', color: AppColors.error)),
        const SizedBox(width: 10),
        Expanded(
            child: _ResultStat(
                label: l.correct,
      ...
      if (masteredNodes.isNotEmpty)
        _TopicList(
            title: l.masteredTopics,
            topics: masteredNodes,
            color: AppColors.success),
      if (failedNodes.isNotEmpty)
        _TopicList(
            title: l.weakTopics, topics: failedNodes, color: AppColors.error),
      const SizedBox(height: 24),
      SizedBox(
          width: double.infinity,
          child: FilledButton.icon(
  ...
  Widget _buildError(BuildContext context, String message) {
    ...
    return Column(children: [
      ...
      FilledButton(
          onPressed: () =>
              context.read<DiagnosticBloc>().add(DiagnosticErrorDismissed()),
        ...
        child: Row(children: [
          ...
          Expanded(
              child: Column(
                ...
                Text(description,
                    style: TextStyle(
                        fontSize: rs(context, 12), color: Colors.grey[400])),
              ])),
          FilledButton(
              onPressed: onStart,
              style: FilledButton.styleFrom(
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Column(children: [
          ...
          Text(label,
              style: TextStyle(
                  fontSize: rs(context, 12), color: Colors.grey[600])),
        ]));
  }
}
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ...topics.map((t) {
            final name =
                t is Map ? (t['name'] ?? t['node_name'] ?? '$t') : '$t';
            return Padding(
