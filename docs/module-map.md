...
**Backend:**
- `core/srez.py` — `pick_srez_problems(..., grade)`: окно difficulty по классу (4→[1,2], 5→[2,3], 6→[3,4], 7→[3,5], NULL→[2,4]), представитель темы по difficulty DESC + 2 стретча сверху; дефицит добирается снизу.
- `core/tutor.py` (+) — `build_system_prompt(ctx, step_n=None)`: блоки МЕТОД УЗЛА (theory_ru через `agent_context.node_theory`) + УЧЕНИК ЗАСТРЯЛ НА ШАГЕ N + 6 жёстких правил (≤3 предложения, встречный вопрос, ответ не называть). `TutorChatIn.step_n ge=1`.
- `db/seed.py` — `backfill_theory` (см. data-state «Идемпотентность»); `scripts/seed_theory.py` — заливка cards-JSON в БД + `--json` запекание в graph-JSON.
- `api/routes.py` — регистрация принимает `grade` (4-7 | null, 400 вне диапазона), `/auth/me` отдаёт. `routers/trainer.py` — WrongTaskOut/`build_wrong_tasks` += `theory_ru` (join nodes), `has_activity` (EXISTS attempts).
**Frontend (webapp/):** auth/`GradeSelect` (обязательный шаг регистрации); hub/`HubOnboarding` (новичок, has_activity=false) vs `HubEmpty` (ветеран); drill/`TheoryCard` («Как решать?», абзацы+`**bold**`+MathText); `TutorPanel` вернулся в drill раскрывашкой «Спросить Кёди» (+step_n активной ступени); фолбэк-ступень для задач без декомпозиции (синтетический rung из task.answer, фото-режим скрыт); нижний PhotoCapture/diagnose-флоу и фейковые STREAK/POINTS удалены. ⚠️ Мёртвый код: `postDiagnose`/`useDiagnose` (FE) + POST `/trainer/diagnose` (BE) без UI-консьюмера — не удалять молча (кандидат на report-fix путь).
