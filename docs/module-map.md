...
| Путь | Назначение |
|------|-----------|
| engine/author-prompt.md | Промпт агента-АВТОРА — простые принципы (Карпатый): «лесенка = шаги ЭТОГО решения», структура якорится на `steps[]`, тест необходимости каждой ступени, заблуждение = ловушка-дистрактор (не отдельная ступень). Канон для чтения/правки (обновлено 2026-06-24) |
| engine/critic-prompt.md | Промпт агента-КРИТИКА — 4 принципа логичности (необходимость / порядок / не-leading / слом=выбор), findings `{rung_index, issue, fix}`, вердикт accept/revise/needs_human (обновлено 2026-06-24) |
| engine/ladder-engine.workflow.js | Оркестратор (Workflow): автор → критик → ≤1 правка на задачу. Вход `args.tasks[]` (grounding=эталонные steps[]). Критик **best-effort** (`.catch`): его падение не теряет author-лесенку (critic:null). Прогон 3 задачи ≈ 1.5 мин |
