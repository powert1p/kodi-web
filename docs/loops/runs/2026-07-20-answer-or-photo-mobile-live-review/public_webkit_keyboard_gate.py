...
def keyboard_geometry(page: Any, *, view: str) -> dict[str, Any]:
    metrics = page.evaluate(
        """() => {
          ...
          const form = document.querySelector('form.learning-actions--form');
          const field = form?.querySelector('input');
          const feedback = document.querySelector('.learning-feedback-status');
          const primary = form?.querySelector('button[data-primary-action]');
          const modeSwitch = [...(form?.querySelectorAll('button') ?? [])]
            .find(element => element.textContent?.trim() === 'Отправить фото');
          const dockNote = document.querySelector('.learning-dock__copy p');
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        with sync_playwright() as playwright:
            ...
            page.locator(".learning-feedback-status").evaluate(
